...
/* OptionNode */
OptionNode* makeOptionNode(unsigned char delta_length, char* option_value);
void addOptionNode(OptionNode* optionNode, unsigned char delta_length, char* option_value);
void initOptionNode(OptionNode* optionNode);
void _initOptionNode(OptionNode* optionNode); /* Don't forgot this for each variable */
OptionNode* getNextOptionNode(OptionNode* optionNode);
OptionNode* get_ithOptionNode(OptionNode* optionNode, int index);
void freeOptionNode(OptionNode * optionNode);
...
void splitPacket(char* buf, int buf_len, Header* header, int* token, OptionNode* optionNode, int* option_num, char* payload, int* paylen);
...
void splitPacket(char* buf, int pkt_len, Header* header, int* token, OptionNode* optionNode, int* option_num, char* payload, int* paylen){
	int len = 0;
	int size;
	int opt_num;
	OptionNode* curOptionNode;
	...
	while (strncmp(&buf[len], "11111111", 8)){
		...
		if (opt_num == 1){
			memcpy(&(optionNode->option), &buf[len], size);
			len += size;
			optionNode->nextOptionNode = NULL;
...
OptionNode* makeOptionNode(unsigned char delta_length, char* option_value){
	OptionNode* optionNode = (OptionNode *)malloc(sizeof(OptionNode));
	if (optionNode == NULL){
		puts("Option Node malloc error!\n");
		exit(1);
	}
	optionNode->nextOptionNode = NULL;
	optionNode->option.delta_length = delta_length;
	memcpy(optionNode->option.option_value, option_value, getOptionLength(delta_length));
...
void _initOptionNode(OptionNode* optionNode){
	optionNode->nextOptionNode = NULL;
}
...
void initOptionNode(OptionNode* optionNode){
	/* initOptionNode(&optionNode), be used before using splitpacket */
	OptionNode* curOptionNode;
	OptionNode* delOptionNode;

	
	curOptionNode = optionNode->nextOptionNode;
	optionNode->nextOptionNode = NULL;
